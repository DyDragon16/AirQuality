...
test.describe("B-0074: Kiểm tra chức năng lọc danh sách người dùng (lọc theo trạng thái Chờ xác nhận)", () => {
	test("Lọc danh sách người dùng theo trạng thái Chờ xác nhận", async ({
		page,
	}) => {
		...
		try {
			...
			// Bước 1 & 2: Chọn trạng thái "Chờ xác nhận"
			await userManagementPage.filterByStatus("pending");

			// Đợi để đảm bảo dữ liệu đã được lọc
			await page.waitForTimeout(2000);

			// Bước 3: Kiểm tra dropdown hiển thị giá trị đã chọn
			await userManagementPage.verifySelectedStatusFilter("pending");

			// Chụp ảnh màn hình kết quả lọc
			await userManagementPage.takeScreenshot("B-0074-pending-users");
			...
			await userManagementPage.verifyAllUsersHaveStatus("Chờ xác nhận");
