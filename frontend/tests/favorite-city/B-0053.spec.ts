...
test.describe("B-0053: Kiểm tra chức năng chỉnh sửa tên nhưng nhập ký tự đặc biệt vào họ", () => {
	test("User không thể chỉnh sửa tên khi nhập ký tự đặc biệt vào họ", async ({
		page,
	}) => {
		...
		try {
			...
			await page.screenshot({
				path: "tests/screenshots/B-0053-before-edit.png",
			});
			...
			// Chụp ảnh popup
			await page.screenshot({
				path: "tests/screenshots/B-0053-edit-modal.png",
			});
			...
			console.log("BƯỚC 3: Đã nhập họ với ký tự đặc biệt: @.~");
			...
			await page.screenshot({
				path: "tests/screenshots/B-0053-filled-form.png",
			});
			...
			if (!isModalStillVisible) {
				...
				if (updatedName?.includes("@") || updatedName?.includes(".~")) {
					console.log(
						"LỖI: Hệ thống đã cho phép cập nhật tên với ký tự đặc biệt trong họ"
					);
					// Chụp ảnh màn hình lỗi
					await page.screenshot({
						path: "tests/screenshots/B-0053-unexpected-update.png",
					});
					// Đảm bảo tên không chứa ký tự đặc biệt
					expect(updatedName).not.toContain("@");
					console.log(
						"Hệ thống không nên cho phép cập nhật khi họ chứa ký tự đặc biệt"
					);
				}
				...
				console.log(
					"Modal đã đóng sau khi nhấn Lưu thay đổi mặc dù họ chứa ký tự đặc biệt"
				);
			}
			...
			const errorSelectors = [
				...
				'text="Họ không được chứa ký tự đặc biệt"',
			...
			if (!foundError) {
				await page.screenshot({
					path: "tests/screenshots/B-0053-missing-validation.png",
				});
				expect(foundError).toBeTruthy();
				console.log(
					"Không hiển thị thông báo lỗi khi nhập ký tự đặc biệt vào họ"
				);
			}

			// Chụp ảnh màn hình thông báo lỗi
			await page.screenshot({
				path: "tests/screenshots/B-0053-error-message.png",
			});
			...
			console.log(
				"Test passed: Tên không bị thay đổi khi nhập ký tự đặc biệt vào họ"
			);
		} catch (error) {
			...
			await page.screenshot({
				path: "tests/screenshots/B-0053-error.png",
			});

			throw error;
		}

		console.log("Test case B-0053 hoàn thành");
	});
});
