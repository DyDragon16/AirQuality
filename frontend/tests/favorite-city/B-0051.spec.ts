...
test.describe("B-0051: Kiểm tra chức năng chỉnh sửa tên nhưng bỏ trống tên", () => {
	test("User không thể chỉnh sửa tên khi bỏ trống tên", async ({ page }) => {
		...
		try {
			...
			await page.screenshot({
				path: "tests/screenshots/B-0051-before-edit.png",
			});
			...
			// Chụp ảnh popup
			await page.screenshot({
				path: "tests/screenshots/B-0051-edit-modal.png",
			});

			// BƯỚC 6: Nhập thông tin mới - Bước 3 và 4 trong yêu cầu
			// Tìm trường nhập Họ và điền "Lê" (bước 3)
			...
			// Tìm trường nhập Tên và xóa nội dung (bước 4)
			const lastNameInput = modalContent.locator("input").nth(1);
			await lastNameInput.fill("");
			console.log("BƯỚC 4: Đã xóa tên, để trống trường tên");

			// Chụp ảnh sau khi điền thông tin
			await page.screenshot({
				path: "tests/screenshots/B-0051-filled-form.png",
			});
			...
			// BƯỚC 8: Kiểm tra thông báo lỗi - Bước 6 trong yêu cầu
			// Đợi thông báo yêu cầu điền vào trường tên
			await page.waitForTimeout(1000);

			// Kiểm tra xem input có thuộc tính 'required' không và có hiển thị thông báo lỗi không
			...
			if (!isModalStillVisible) {
				...
				// Nếu tên đã thay đổi, test phải fail vì không nên cho phép cập nhật khi trường tên trống
				if (updatedName !== currentName) {
					console.log(
						"LỖI: Hệ thống đã cho phép cập nhật tên mặc dù trường tên bị bỏ trống"
					);
					// Chụp ảnh màn hình lỗi
					await page.screenshot({
						path: "tests/screenshots/B-0051-unexpected-update.png",
					});
					expect(updatedName).toBe(currentName);
					console.log(
						"Hệ thống không nên cho phép cập nhật khi tên bị bỏ trống"
					);
				}
				...
				console.log(
					"Modal đã đóng sau khi nhấn Lưu thay đổi mặc dù trường tên bị bỏ trống"
				);
			}

			// Hiển thị yêu cầu validation trên UI (nếu có)
			if (isLastNameRequired) {
				console.log(
					"Trường tên có thuộc tính required, sẽ hiển thị thông báo lỗi của trình duyệt"
				);
			} else {
				...
				if (!foundError) {
					await page.screenshot({
						path: "tests/screenshots/B-0051-missing-validation.png",
					});
					expect(foundError).toBeTruthy();
					console.log("Không hiển thị thông báo lỗi khi bỏ trống trường tên");
				}
			}

			// Chụp ảnh màn hình thông báo lỗi
			await page.screenshot({
				path: "tests/screenshots/B-0051-error-message.png",
			});

			// BƯỚC 9: Đóng modal bằng nút Hủy
			...
			// BƯỚC 10: Kiểm tra tên không bị thay đổi
			...
			console.log("Test passed: Tên không bị thay đổi khi bỏ trống tên");
		} catch (error) {
			console.log(`Lỗi trong quá trình test: ${error}`);

			// Chụp ảnh màn hình khi có lỗi
			await page.screenshot({
				path: "tests/screenshots/B-0051-error.png",
			});

			throw error;
		}

		console.log("Test case B-0051 hoàn thành thành công");
	});
});
